...
const int positions[NOMBRE_SERVO][2] = {
  ...
  {140, 450}, // 09 : B, haut, planche


  {130, 440}, // 10 : B, haut, GG
  {255, 445}, // 11 : B, haut, G
  {135, 330}, // 12 : B, haut, D
  {200, 460}  // 13 : B, haut, DD
};
...
void Change_valeur_Servo(int indiceServo){

  while(1){
    Serial.println("Entrer la nouvelle valeur du servo (RETURN pour sortir) : ");
    
    while (!Serial.available()) {
      // attendre une nouvelle entrée utilisateur
    }
    ...
    int NouvellePos = b.toInt();
    pwm.setPWM(indiceServo, 0, NouvellePos);
    Serial.println("Nouvelle Position envoyée !");
  }
}
...
void Set_pos_Servo(){
  ...
  while(1){
    ...
    Serial.println("Entrer l'ID d'un servo (Press 'ENTER' to exit): ");
    while (!Serial.available()) {
      // attendre qu'une donnée arrive
    }
    ...
    int valeurRecue = c.toInt();
    c = "";
    Change_valeur_Servo(valeurRecue);
  }
}
...
void switchCase(int valeurUART){
  ...
  switch (valeurUART){
    ...
    case 9999:
      Serial.println("Set_pos_Servo");
      Set_pos_Servo();
      Serial.println("FIN SET POS");
      break;
